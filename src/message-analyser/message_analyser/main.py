...
def analyse(df: pd.DataFrame, crime_elements: str, results_dir: Path) -> Path:
    ...
    with open(output_file, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["conversation_id", "chunk_id", "message_number", "speaker", "message", "crime_element"]
        )
        writer.writeheader()
...
def analyze_conversations(inputs: CrimeAnalysisInputs, parameters: CrimeAnalysisParameters) -> ResponseBody:
    ...
    try:
        ...
        error_file = RESULTS_DIR / "error_log.txt" if RESULTS_DIR else Path("error_log.txt")
